...
class UpYunStorage(Storage):
    ...
    UpYun Storage
    """
    def __init__(self, account=None, password=None, bucket=None, prefix=None):
        ...
        self.prefix = prefix or settings.UPYUN_PREFIX
        self.api_url = "http://v0.api.upyun.com"
        self.cache = {}

    def _endpoint(self, bucket_name):
        if self.prefix:
            return "%s/%s/%s/%s" % (self.api_url, self.bucket, self.prefix, bucket_name)
        else:
